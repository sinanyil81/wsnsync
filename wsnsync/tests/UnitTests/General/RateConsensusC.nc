...
*/
    ...
    command void RateConsensus.print(){
        int i,j;
        for (i = 0; i < MAX_NEIGHBORS; i++) {
            if(neighbors[i].state == ENTRY_FULL){
                DEBUG_PRINT("___________________________\n");
                DEBUG_PRINT("Neighbor ID = %x \n",neighbors[i].id);
                DEBUG_PRINT("Timestamp = %x \n",neighbors[i].timestamp);
                DEBUG_PRINT("rate = %f \n",neighbors[i].multiplier);
                DEBUG_PRINT("relative rate = %f \n",neighbors[i].skew);
                DEBUG_PRINT("localAverage = %x \n",neighbors[i].localAverage);
                DEBUG_PRINT("offsetAverage = %x \n",neighbors[i].offsetAverage);
                DEBUG_PRINT("tableEntries = %x \n",neighbors[i].tableEntries);
                DEBUG_PRINT(".................\n");
                for (j = 0; j < MAX_ENTRIES; j++) {
                    if(neighbors[i].table[j].state == ENTRY_FULL){
                        DEBUG_PRINT("X = %x (Y-X) = %x \n",neighbors[i].table[j].localTime,neighbors[i].table[j].timeOffset);
                    }
                }
                DEBUG_PRINT("___________________________\n");
            }
