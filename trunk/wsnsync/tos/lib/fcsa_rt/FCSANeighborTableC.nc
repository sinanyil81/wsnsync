...
*/
    ...
    command void FCSANeighborTable.reset(){
        clearNeighbors();
    }

    command void FCSANeighborTable.update(uint32_t time){
        int8_t i;
        uint32_t age;
    ...
    command error_t FCSANeighborTable.storeInfo(uint16_t id,float multiplier,float rootMultiplier,uint32_t localTime,uint32_t globalTime,uint32_t timestamp){
    ...
    command void FCSANeighborTable.getNeighborhoodRate(float *myRate){
    ...
    async command void FCSANeighborTable.getNeighborhoodTime(uint32_t *myClock,uint32_t timestamp){
        uint8_t i;
		int64_t diffSum = 0;
